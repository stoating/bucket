  ...
  (testing "metadata with :stdout output"
    (let [meta-data {:user "alice" :timestamp 1234567890}
      ...
      (is (.contains out-str "1234567890")
          "print-meta writes formatted metadata to stdout"))))

(deftest print-meta-to-file-test
  (testing "metadata with :file output"
    (let [meta-data {:operation "test" :version "1.0"}
          ...
          out-str (with-out-str
                    (logging/print-meta meta-data :out :file :name bucket-name :dir temp-dir))]
      ...
      (is (= 1 (count (.listFiles temp-dir-file)))
          "print-meta creates a single .edn file in the specified directory"))))

(deftest print-meta-to-both-test
  (testing "metadata with :both output"
    (let [meta-data {:env "prod" :tags ["test"]}
          ...
          temp-dir-file (io/file temp-dir)
          out-str (with-out-str
                    (logging/print-meta meta-data :out :both :name bucket-name :dir temp-dir))]
      (is (.contains out-str "=== Bucket Metadata ==="))
      ...
      (is (= 1 (count (.listFiles temp-dir-file)))
          "print-meta outputs to stdout and creates file when :both is specified"))))

(deftest print-meta-to-none-test
  (testing "metadata with :none output"
    (let [meta-data {:should-not-appear "anywhere"}
          ...
          temp-dir-file (io/file temp-dir)
          out-str (with-out-str
                    (logging/print-meta meta-data :out :none :name bucket-name :dir temp-dir))]
      (is (= "" out-str))
      (is (zero? (count (.listFiles temp-dir-file)))
          "print-meta produces no output when :none is specified"))))

(deftest print-meta-complex-structure-test
  (testing "nested metadata structures"
    (let [meta-data {:user {:name "bob" :id 42}
          ...
          out-str (with-out-str
                    (logging/print-meta meta-data :out :stdout :name "complex-bucket"))]
      (is (.contains out-str "=== Bucket Metadata ==="))
      (is (.contains out-str ":user"))
      ...
      (is (.contains out-str "important")
          "print-meta handles nested maps and collections in metadata"))))

(deftest print-meta-with-custom-name-test
  (testing "custom bucket name in file output"
    (let [meta-data {:custom "name"}
          ...
          temp-dir-file (io/file temp-dir)
          out-str (with-out-str
                    (logging/print-meta meta-data :out :file :name custom-name :dir temp-dir))]
      (is (.contains out-str "Metadata written to:"))
      ...
      (let [created-file (first (.listFiles temp-dir-file))]
        (is (.contains (.getName created-file) custom-name)
            "print-meta uses the :name parameter in the output filename")))))
