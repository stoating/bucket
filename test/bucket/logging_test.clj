...
(deftest make-entry-test
  (testing "make-entry with different arities"
    (testing "single argument"
      (let [entry (logging/make-entry "test message")]
    ...
    (testing "message and level"
      (let [entry (logging/make-entry "test message" :level :error)]
        (is (= "test message" (:value entry)))
    ...
    (testing "message, level, and indent"
      (let [entry (logging/make-entry "test message" :level :warning :indent 3)]
        (is (= "test message" (:value entry)))
...
(deftest likely-password?-test
  (testing "exact matches"
    (are [input expected] (= expected (logging/likely-password? input))
      "confidential" true
      "Confidential" true
      "CONFIDENTIAL" true
      "encrypted" true
  ...
  (testing "log function with different arities"
    (let [initial-logs []]
      (testing "basic logging"
        (let [logs (logging/log initial-logs "test message")]
          (is (= 1 (count logs)))
      ...
      (testing "logging with level"
        (let [logs (logging/log initial-logs "error message" :level :error)]
          (is (= 1 (count logs)))
          (let [entry (first logs)]
      ...
      (testing "logging with level and indent"
        (let [logs (logging/log initial-logs "indented message" :level :warning :indent 2)]
          (is (= 1 (count logs)))
          (let [entry (first logs)]
      ...
      (testing "indent inheritance"
        (let [logs (-> initial-logs
                       (logging/log "first message" :level :info :indent 3)
                       (logging/log "second message"))]
          (is (= 2 (count logs)))
      ...
      (testing "password redaction"
        (let [logs (logging/log initial-logs "password=secret123" :check-pass true)]
          (is (= 1 (count logs)))
          (let [entry (first logs)]
...
(deftest format-log-message-test
  (testing "log message formatting"
    (let [test-time (Instant/parse "2023-12-01T10:30:45Z")
          ...
          formatted (logging/format-log-message entry)]
...
(deftest print-logs-test
  (testing "printing logs to output stream"
    (let [logs [(logging/make-entry "first message")
                (logging/make-entry "second message" :level :error :indent 1)]
          result (with-out-str
                   (logging/print-logs logs :out :stdout))]
...
(deftest with-indent-test
  (testing "adding indentation to logs"
    (let [logs [(logging/make-entry "message1" :indent 1)
                (logging/make-entry "message2" :level :error :indent 2)]
          indented-logs (logging/with-indent logs 3)]
...
(deftest filter-by-level-test
  (testing "filtering logs by minimum level"
    (let [logs [(logging/make-entry "debug msg" :level :debug)
                (logging/make-entry "info msg" :level :info)
                (logging/make-entry "warning msg" :level :warning)
                (logging/make-entry "error msg" :level :error)
                (logging/make-entry "critical msg" :level :critical)]]
      (testing "filter by debug (should include all)"
        (let [filtered (logging/filter-by-level logs :debug)]
          (is (= 5 (count filtered))
              "debug level includes all logs")))

      (testing "filter by info (should exclude debug)"
        (let [filtered (logging/filter-by-level logs :info)]
          (is (= 4 (count filtered)))
          (is (every? #(not= :debug (:level %)) filtered)
              "info level excludes debug logs")))

      (testing "filter by warning"
        (let [filtered (logging/filter-by-level logs :warning)]
          (is (= 3 (count filtered)))
...
(deftest convenience-functions-test
  ...
  (testing "warning function"
    (let [logs (logging/warning [] "warning message")]
      (is (= 1 (count logs)))
      (is (= :warning (:level (first logs))))
  ...
  (testing "error function"
    (let [logs (logging/error [] "error message")]
      (is (= 1 (count logs)))
  ...
  (testing "critical function"
    (let [logs (logging/critical [] "critical message")]
      (is (= 1 (count logs)))
  ...
  (testing "logging behavior in CLI context"
    (testing "log accumulation across multiple operations"
      (let [logs (-> []
                     ...
                     (logging/debug "Processing step 1" :indent 2)
                     (logging/debug "Processing step 2" :indent 2)
                     (logging/warning "Minor issue detected" :indent 2)
    ...
    (testing "log filtering for different verbosity levels"
      (let [logs [(logging/make-entry "debug info" :level :debug)
                  (logging/make-entry "general info" :level :info)
                  (logging/make-entry "warning" :level :warning)
                  (logging/make-entry "error occurred" :level :error)]
            info-and-above (logging/filter-by-level logs :info)
            warning-and-above (logging/filter-by-level logs :warning)]
    ...
    (testing "password redaction in CLI logging"
      (let [logs (-> []
                     (logging/log "Starting authentication")
                     (logging/log "api_key=secret123" :check-pass true)
                     (logging/log "Authentication successful"))]
        (is (= 3 (count logs)))
...
(deftest log-output-format-test
  (testing "CLI-compatible log output format"
    (let [entry (logging/make-entry "CLI operation completed")
          printed-output (with-out-str
                           (logging/print-logs [entry] :out :stdout))]
...
(deftest log-file-output-test
  (testing "print-logs writes to file with :out :file"
    (let [logs [(logging/make-entry "First log message")
                (logging/make-entry "Second log message" :level :error :indent 1)]
          test-dir (str th/test-temp-root "/logs")]
      (logging/print-logs logs :out :file :dir test-dir :timestamp false :name "test")
  ...
  (testing "print-logs writes to file with timestamp"
    (let [logs [(logging/make-entry "Timestamped log")]
          test-dir (str th/test-temp-root "/logs")]
      (logging/print-logs logs :out :file :dir test-dir :timestamp true)
  ...
  (testing "print-logs with :out :both writes to file and stdout"
    (let [logs [(logging/make-entry "Both output log" :level :warning)]
          test-dir (str th/test-temp-root "/logs")
          stdout-output (with-out-str
                          (logging/print-logs logs :out :both :dir test-dir :timestamp false :name "both-test"))]
  ...
  (testing "print-meta writes to file"
    (let [meta {:bucket-name "test-bucket" :total-items 42}
      ...
      (logging/print-meta meta :out :file :dir test-dir :timestamp false :name "test-meta")
  ...
  (testing "print-meta with :out :both writes to file and stdout"
    (let [meta {:test-key "test-value"}
          test-dir (str th/test-temp-root "/meta")
          stdout-output (with-out-str
                          (logging/print-meta meta :out :both :dir test-dir :timestamp false :name "both-meta"))]
